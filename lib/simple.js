...
exports.makeWASocket = (connectionOptions, options = {}) => {
  ...
  conn.ev.on("messages.update", async (chatUpdate) => {
    async function loadMessage(messageDetails) {
      if (store) {
        const message = await store.loadMessage(messageDetails.remoteJid, messageDetails.id)
        return message?.["message"]
      }
      return {
        conversation: "Hi Im Kasumi Bot"
      }
    }
  ...
  for (const { 
    ...
    if (update.pollUpdates && key.fromMe) {
      console.log('Detect Poll Message')
      const pollCreation = await loadMessage(key)
      if (pollCreation) {
        const PollUpdates = await getAggregateVotesInPollMessage({
          message: pollCreation,
          pollUpdates: update.pollUpdates
        })
        const toCmd = pollUpdates.filter(v => v.voters.length !== 0)[0]?.name
        if (toCmd === undefined) {
          return
        }
        const namePrefix = /*global.db.setting.prefix ? global.db.setting.prefix : '' + */ '.' + toCmd
        conn.appenTextMessage(namePrefix, chatUpdate)
  ...
  conn.sendPoll = async (jid, name = '', values = [], quoted, selectableCount = 1, options = {}) => { 
  ...
  conn.parseMention = (text) => [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(match => match[1] + "@s.whatsapp.net")
        
  /** 
    * Don't delete it, it can cause errors!!! 
    * Thanks for this code Kens Ransyahh
  **/
  ...
  conn.sendMessageModify = async (jid, text, quoted, properties = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    if (properties.thumbnail) {
      var { file } = await Func.getFile(properties.thumbnail)
    }
    return conn.generateMessage(jid, { text,
      contextInfo: {
        ...
        externalAdReply: {
          ...
          thumbnail: properties.thumbnail ? await Func.fetchBuffer(file) : await Func.fetchBuffer(global.db.data.settings[conn.user.jid].cover),
  ...
  **/
  ...
  conn.resize = async (anu, ini, itu) => {
    let oyy = await Jimp.read(anu)
    let kyomasa = await oyy.resize(ini, itu).getBufferAsync(Jimp.MIME_JPEG)
    return kyomasa
  }
  
  conn.sendProgress = async (jid, text, quoted) => {
    const arr = [
      { text: "⬡⬡⬡⬡⬡⬡⬡⬡⬡⬡ 0%", timeout: 600 },
      { text: "⬢⬡⬡⬡⬡⬡⬡⬡⬡⬡ 7%", timeout: 600 },
      { text: "⬢⬢⬡⬡⬡⬡⬡⬡⬡⬡ 16%", timeout: 600 },
      ...
      { text: "⬢⬢⬢⬢⬢⬢⬢⬢⬡⬡ 84%", timeout: 600 },
      { text: "⬢⬢⬢⬢⬢⬢⬢⬢⬢⬡ 97%", timeout: 600 },
      { text: "⬢⬢⬢⬢⬢⬢⬢⬢⬢⬢ 100%", timeout: 600 },
      { text: text, timeout: 600 },
    ]
    const anu = await conn.sendMessage(jid, { text: 'Loading . . .' }, { quoted });
  ...
  **/
  /*conn.reply = async (jid, text = '', quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    const externalAdReply = options.externalAdReply || {
      title: 'Kasumi shirogane',
      body: 'Simple bot whatsapp commondJs',
      thumbnailUrl: 'https://pomf2.lain.la/f/n4efohz.jpg',
      mediaType: 1,
      mediaUrl: global.media.sfb,
      sourceUrl: global.media.sfb
    }
  ...
  conn.reply = async (jid, text = '', quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    if (options.useModify) {
      const properties = {
        title: options.title || 'Kasumi Shirogane',
        body: options.body || '',
        thumbnail: options.thumbnail || 'https://pomf2.lain.la/f/n4efohz.jpg',
        largeThumb: options.largeThumb || false,
        url: options.url || '',
      }
      return conn.sendMessageModify(jid, text, quoted, properties)
    } else {
      return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, {
        ...options,
        text,
  ...
  **/
  ...
  conn.sendFMessage = async (jid, headerTitle, text, footer, media, quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    let preparedMedia = {}
    if (media) {
      preparedMedia = await prepareWAMessageMedia({ image: { url: media } }, { upload: conn.waUploadToServer })
    }
    const msg = await generateWAMessageFromContent(jid, {
      interactiveMessage: {
        ...
        contextInfo: {
          ...
          isForwarded: false,
          mentionedJid: conn.parseMention(`${text || ""} ${footer || ""} ${headerTitle || ""}`),
        },
        externalAdReply: {
  ...
  conn.sendIAMessage = async (jid, btns = [], qoted, opts = {}) => {
    let messageContent = {
      viewOnceMessage: {
        message: {
          messageContextInfo: {
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.message) {
    ...
    /*if (m.mtype === "pollUpdateMessage") {
      const poll = m.message.pollUpdateMessage
      const store = store.loadMessage(m.chat, m.message.pollUpdateMessage.pollCreationMessageKey.id)
      m.update = updateMessageWithPollUpdate(store, poll)
      const pollUpdate = getAggregateVotesInPollMessage(store, conn.user.id)
      m.pollUpdates = pollUpdate
    }*/
    ...
    if (m.quoted) {
      ...
      /*if (["pollCreationMessage"].includes(type)) {
        let store = store.loadMessage(m.chat, m.msg.contextInfo.stanzaId)
        let mPollUptate = getAggregateVotesInPollMessage(store, global.conn.user.id)
        m.quoted.pollUpdates = mPollUptate
      }*/
